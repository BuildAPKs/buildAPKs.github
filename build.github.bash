...
_MAINGITHUB_ () {
	...
	then	# create null directory, profile, repos files, and exit
		...
		awk 'NR>=16 && NR<=42' "$RDR/opt/db/README.md" || printf "\\e[7;38;5;203m%s\\e[0m\\n" "Running \` awk 'NR>=16 && NR<=42' $RDR/opt/db/README.md \` failed; Continuing...  "
...
_PRINTJS_ () {
	printf "\\n\\e[1;34mSearching for C* Java* Kotlin Objective-C* Pearl Python R* and Shell language repositories: "'\033]2;Searching for C* Java* Kotlin Objective-C* Pearl Python R* and Shell language repositories: OK\007'
}

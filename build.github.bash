...
_MAINGITHUB_ () {
	...
	then	# create null directory, profile, repos files, and exit
		...
		awk 'NR>=16 && NR<=46' "$RDR/opt/db/README.md" || printf "\\e[7;38;5;203m%s\\e[0m\\n" "Running \` awk 'NR>=16 && NR<=42' $RDR/opt/db/README.md \` failed; Continuing...  "
	...
	_GETREPOS_
	_PRINTJS_
	JARR=($(grep -B 5 -e "\"\:\ \"Java" -e "\"\:\ \"Objective-C" -e "\"\:\ \"R" -e "\"\:\ \"C\"" -e "\"\:\ \"C#\"" -e "\"\:\ \"C++\"" -e "\"\:\ \"Kotlin"\" -e "\"\:\ \"Pearl"\" -e "\"\:\ \"Python"\" -e "\"\:\ \"Shell\"" "$JDR/repos" | grep svn_url | awk -v x=2 '{print $x}' | sed 's/\,//g' | sed 's/\"//g')) || _SIGNAL_ "402" "create JARR ${0##*/} build.github.bash" # create array of C C# C++ Java* Kotlin Objective-C* Pearl Python R* and Shell language repositories
	_PRINTJD_
	if [[ "${JARR[@]}" == *ERROR* ]]
	then

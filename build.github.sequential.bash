...
export JDR="$RDR/sources/github/sequential/$TOPIC"
...
printf "\\n\\e[1;38;5;116m%s\\n\\e[0m" "${0##*/}: Beginning BuildAPKs with build.github.sequential.bash $@:"
[ ! -d "$JDR" ] && mkdir -p "$JDR"
if [[ ! -f "$JDR"/sequential ]] 
then
	...
	then	# use https://github.com/settings/tokens to create tokens
		...
		do	# get a page of repository information
			...
			curl -u "$OAUT" -H "Accept: application/vnd.github.mercy-preview+json" "https://api.github.com/search/repositories?per_page=100&page=$RPCT&q=topic:$TOPIC+language:Java&sort=forks&order=desc" > "$JDR/sequential.tmp"
			cat "$JDR/sequential.tmp" >> "$JDR/sequential"  
			RPCT="$(($RPCT-1))"
			sleep "0.${RANDOM::3}"
		done
	else
		...
		do	# get a page of repository information
			printf "%s\\n" "Downloading GitHub $TOPNAME page $RPCT topic repositories information: "
			curl -H "Accept: application/vnd.github.mercy-preview+json" "https://api.github.com/search/repositories?per_page=100&page=$RPCT&q=topic:$TOPIC+language:Java&sort=stars&order=desc" > "$JDR/sequential.tmp"
			cat "$JDR/sequential.tmp" >> "$JDR/sequential"  
			RPCT="$(($RPCT-1))"
			sleep "0.${RANDOM::3}"
		done
	fi
	rm -f "$JDR/sequential.tmp"
fi
SQARR=($(grep -B 5 -e Java -e JavaScript -e Shell -e Kotlin "$JDR/sequential" | grep svn_url | awk -v x=2 '{print $x}' | sed 's/\,//g' | sed 's/\"//g' | sed 's/https\:\/\/github.com\///g' | cut -d\/ -f1)) ||: # create array of Java, JavaScript, Shell and Kotlin Java language repositories	
for NAME in "${SQARR[@]}" 
do 
